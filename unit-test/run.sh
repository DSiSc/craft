...
packages_diff() {
    git -C "${GOPATH}/src/github.com/DSiSc/craft" diff --no-commit-id --name-only -r "${1:-HEAD}" |
        grep '.go$' | grep -Ev '^vendor/|^build/' | \
        sed 's%/[^/]*$%/%' | sort -u | \
        awk '{print "github.com/DSiSc/craft/"$1"..."}'
}
...
main() {
    ...
    local package_spec=${TEST_PKGS:-github.com/DSiSc/craft/...}
